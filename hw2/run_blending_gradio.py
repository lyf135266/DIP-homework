...
def create_mask_from_points(points, img_h, img_w):
    ...
    # Convert points to the correct format for cv2
    points = points.reshape((-1, 1, 2)).astype(np.int32)

    # Draw the polygon on the mask using cv2
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_kernel = torch.tensor([[1,  1, 1],
                                      [1, -8, 1],
                                      [1,  1, 1]], device=foreground_img.device, dtype=torch.float).unsqueeze(0).unsqueeze(0)

    # Duplicate the kernel for all three channels
    laplacian_kernel = laplacian_kernel.repeat(1, 3, 1, 1)

    # Compute the Laplacian of the blended image
    ...
    # Apply masks
    fg_masked = foreground_mask * foreground_laplacian
    bg_masked = background_mask * blended_laplacian

    # 计算损失
    loss = F.mse_loss(fg_masked, bg_masked)
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    for i in range(m):
        for j in range(n):
            ...
            # 检查是否在目标图像的边界内
            if 0 <= new_i < foreground_np.shape[0] and 0 <= new_j < foreground_np.shape[1]:
                foreground_np[new_i, new_j] = foreground_np0[i, j]
    ...
    foreground_mask = create_mask_from_points(background_polygon_points, foreground_np.shape[0], foreground_np.shape[1])
...
def close_polygon_and_reset_dx(img_original, polygon_state, dx, dy, background_image_original):
    ...
    updated_background = update_background(background_image_original, updated_polygon_state, dx, dy)
